...
def test_foamcase_nonexistent_path() -> None:
...
def test_foamcase_empty_directory() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Should NOT raise when accessing the property (returns FoamFile object)
        control_dict = case.control_dict
        assert control_dict.path.name == "controlDict"

        # Should raise when trying to access the content of missing files
        with pytest.raises(FileNotFoundError):
            _ = control_dict["startTime"]


def test_foamcase_time_directory_edge_cases() -> None:
...
def test_time_directory_field_access() -> None:
...
def test_time_directory_compressed_vs_uncompressed() -> None:
...
def test_foamcase_file_access() -> None:
...
def test_foamcase_indexing_edge_cases() -> None:
...
def test_foamcase_boolean_evaluation() -> None:
...
def test_foamcase_representation() -> None:
...
def test_time_directory_representation() -> None:
...
def test_foamcase_with_complex_time_structure() -> None:
...
def test_foamcase_file_method() -> None:
...
def test_time_directory_length_and_membership() -> None:
...
def test_foamcase_clone_context_manager() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Test clone context manager with explicit destination
        clone_dst = case_path.parent / "clone"
        with case.clone(clone_dst) as clone:
...
def test_foamcase_clone_basic() -> None:
    """Test FoamCase.clone() basic functionality."""
    with tempfile.TemporaryDirectory() as tmpdir:
        case_path = Path(tmpdir) / "original_case"
        case_path.mkdir()
        ...
        # Clone to explicit destination
        clone_dst = case_path.parent / "cloned_case"
        clone = case.clone(clone_dst)

        assert (clone.path / "0" / "U").exists()
        assert (clone.path / "processor0").exists()
...
def test_foamcase_properties_with_missing_files() -> None:
