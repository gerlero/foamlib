...
def test_issue_393_complex_keys():
    ...
    """)
    assert parsed[("divSchemes", "div((muEff*dev2(grad(U).T())))")] == ("Gauss", "linear")
    ...
    """)
    assert parsed[("divSchemes", "div(((rho*nuEff)*dev2(T(grad(U)))))")] == ("Gauss", "linear")
    ...
    """)
    assert parsed[("laplacianSchemes", "laplacian((rho*DomegaEff),omega)")] == ("omegaGauss", "linear", "limited", 0.333)
    ...
    """)
    assert parsed[("laplacianSchemes", "laplacian((rho*DkEff),k)")] == ("Gauss", "linear", "limited", 0.333)
    assert parsed[("laplacianSchemes", "laplacian((rho*DepsilonEff),omega)")] == ("Gauss", "linear", "limited", 0.333)
    assert parsed[("laplacianSchemes", "laplacian((rho*DnuTildaEff),k)")] == ("Gauss", "linear", "limited", 0.333)


def test_original_working_cases_still_work():
    ...
    assert parsed[("divSchemes", "div(phi,omega)")] == ("Gauss", "linearUpwind", "grad(omega)")
    ...
    assert parsed[("laplacianSchemes", "default")] == ("Gauss", "linear", "limited", 0.333)