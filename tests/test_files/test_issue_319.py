...
def test_subdictionary_update_no_blank_line_accumulation():
    """Test that FoamFile.update() doesn't accumulate blank lines in sub-dictionaries."""
    
    # Import here to avoid import issues in test environments
    import sys
    # Adjust this path as needed for the actual test environment
...
"""
    ...
    try:
        ...
        assert second_line_idx is not None, "Could not find second_line in initial content"
        
        # Perform multiple updates and ensure blank lines don't accumulate
        blank_line_counts = []
        
        for i in range(5):
            testDict = FoamFile(temp_path)
...
if __name__ == "__main__":
    test_subdictionary_update_no_blank_line_accumulation()
    test_subdictionary_direct_assignment_no_blank_line_accumulation()
    print("✓ All tests for issue #319 passed")