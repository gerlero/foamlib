...
def test_foamfile_header_edit_preserves_formatting(tmp_path: Path) -> None:
    ...
    assert 'format' not in version_line, "version and format should not be on the same line"
...
def test_subdictionary_edit_preserves_formatting(tmp_path: Path) -> None:
    ...
    lines = content.split('\n')
    subkey1_line = [line for line in lines if 'subkey1' in line][0]
    assert 'subkey2' not in subkey1_line, "subkey1 and subkey2 should not be on the same line"
...
def test_nested_subdictionary_edit_preserves_formatting(tmp_path: Path) -> None:
    ...
    lines = content.split('\n')
    key_line = [line for line in lines if 'key' in line and 'newvalue' in line][0]
    # The key should start with at least some whitespace (4 spaces minimum)
    assert key_line.startswith("    "), "nested key should have proper indentation (at least 4 spaces)"
