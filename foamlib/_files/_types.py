...
from typing import Any, NamedTuple, TypeAlias, TypeGuard
...
Field: TypeAlias = (
    float | np.ndarray[tuple[int] | tuple[int, int], np.dtype[np.float64 | np.float32]]
)
...
StandaloneData: TypeAlias = (
    ...
    | list[np.ndarray[tuple[int, int], np.dtype[np.float64 | np.float32]]]
    | tuple[
        np.ndarray[tuple[int], np.dtype[np.int64 | np.int32]],
        np.ndarray[tuple[int], np.dtype[np.int64 | np.int32]],
    ]
